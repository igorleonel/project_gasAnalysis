                ...
                dbc.CardBody([
                    ...
                    dbc.Row([
                        dbc.Col([
                            ...
                            html.Legend('Igor Leonel')
                        ])
                    ], style={'margin-top': '10px'}),
                    dbc.Row([
                        dbc.Button('Portfólio', href='https://igorleonel.github.io/portfolio_projetos/', target='_blank')
                    ], style={'margin-top': '10px'})
                ])
            ], style=tab_card)
        ], sm=4, lg=2),
...
def graph1(data, ano, regiao, toggle):
    ...
    dff_regiao['VALOR REVENDA (R$/L)'] = dff_regiao['VALOR REVENDA (R$/L)'].round(2)
    dff_estado['VALOR REVENDA (R$/L)'] = dff_estado['VALOR REVENDA (R$/L)'].round(2)

    fig1_text = [f'{x} - R${y}' for x, y in zip(dff_regiao['REGIÃO'].unique(), dff_regiao['VALOR REVENDA (R$/L)'].unique())]
    fig2_text = [f'R${y} - R${x}' for x, y in zip(dff_estado['ESTADO'].unique(), dff_estado['VALOR REVENDA (R$/L)'].unique())]
